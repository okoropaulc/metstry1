...
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs, truncLen=c(200,150), maxN=0, matchIDs = TRUE, maxEE=c(2,2), truncQ=2, rm.phix=TRUE, compress=TRUE, multithread=TRUE)
head(out)
...
#Construct the sequence table Also called the OTU Table
...
head(track)
write.table(track, file ="/home/paul/metstry1/metstry2/Third/mergersReport.txt", sep="\t", quote = F)
...
write.table(asv_tax, "/home/paul/metstry1/metstry2/Third/ASVs_taxonomy.txt", sep="\t", quote=F)
...
write(asv_fasta, "/home/paul/metstry1/metstry2/Third/ASVs.fa")

# count table or OTU table:
asv_tab <- t(seqtab.nochim)
row.names(asv_tab) <- sub(">", "", asv_headers)
write.table(asv_tab, "/home/paul/metstry1/metstry2/Third/ASVs_counts.txt", sep="\t", quote=F)


# taxa table with species:
asv_tax <- taxa.print
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "/home/paul/metstry1/metstry2/Third/ASVs_taxonomy_with_Species.txt", sep="\t", quote=F)

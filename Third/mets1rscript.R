...
#sq.R1 <- substr(getSequences(seqtab.nochim), 1, R1_READ_LEN)
#gen.spc <- assignSpecies(sq.R1, "/home/paul/silva_species_assignment_v128.fa.gz")
...
write.table(asv_tab, "/home/paul/metstry1/Third/ASVs_counts.txt", sep="\t", quote=F)

# taxa table with no species assignment:
asv_tax <- taxa
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "/home/paul/metstry1/Third/ASVs_taxonomy.txt", sep="\t", quote=F)

# taxa table with species:
asv_tax <- taxa.print
row.names(asv_tax) <- sub(">", "", asv_headers)
write.table(asv_tax, "/home/paul/metstry1/Third/ASVs_taxonomy_with_Species.txt", sep="\t", quote=F)
